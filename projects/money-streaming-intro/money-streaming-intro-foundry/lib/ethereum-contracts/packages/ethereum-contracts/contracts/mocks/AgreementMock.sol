...
contract AgreementMock is AgreementBase {
    ...
    modifier requireValidCtx(bytes memory ctx) {
        require(
            ...
            "AgreementMock: ctx not valid given by host?!"
        );
        _;
    }
    ...
    function _callAppAfterAgreementCallback(
        ...
        if (isJailed) {
            ...
            require(
                appContext.callType == 0,
                "AgreementMock: callback should not reach jailed app"
            );
        } else {
            require(
                ISuperfluid(msg.sender).isCtxValid(newCtx),
                "AgreementMock: ctx not valid after callback"
            );
        }
