...
interface ITOGAv1 {
    ...
    function getCurrentPIC(
        ISuperToken token
    ) external view returns (address pic);
    ...
    function getCurrentPICInfo(
        ISuperToken token
    ) external view returns (address pic, uint256 bond, int96 exitRate);
    ...
    function getDefaultExitRateFor(
        ISuperToken token,
        uint256 bondAmount
    ) external view returns (int96 exitRate);
...
contract TOGA is ITOGAv2, IERC777Recipient {
    ...
    uint64 internal constant ERC777_SEND_GAS_LIMIT = 3000000;
    ...
    function getCurrentPIC(
        ISuperToken token
    ) external view override returns (address pic) {
    ...
    function getDefaultExitRateFor(
        ISuperToken /*token*/,
        uint256 bondAmount
    ) public view override returns (int96 exitRate) {
    ...
    function getMaxExitRateFor(
        ISuperToken /*token*/,
        uint256 bondAmount
    ) external view override returns (int96 exitRate) {
    ...
    function changeExitRate(
        ...
        if (curExitRate > 0 && newExitRate > 0) {
            ...
            _host.callAgreement(
                ...
                abi.encodeCall(
                    _cfa.deleteFlow,
                    (token, address(this), currentPICAddr, new bytes(0))
                ),
                "0x"
            );
    ...
    function _getCurrentPICBond(
        ISuperToken token
    ) internal view returns (uint256 bond) {
