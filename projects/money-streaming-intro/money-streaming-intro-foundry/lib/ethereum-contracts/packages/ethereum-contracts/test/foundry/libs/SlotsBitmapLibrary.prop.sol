...
import {
    SlotsBitmapLibrary
} from "@superfluid-finance/ethereum-contracts/contracts/libs/SlotsBitmapLibrary.sol";
import {
    ISuperToken
...
contract SlotsBitmapLibraryProperties is Test {
    ...
    function testCreateConsequtiveSlotsAndRemoveOneSlot(
        uint8 _numSlots,
        uint8 _subIdToRemove
    ) public {
        ...
        _subIdToRemove = uint8(
            bound(uint256(_subIdToRemove), 0, _numSlots - 1)
        );
    ...
    function _slotExists(
        uint32[] memory _slotIds,
        uint8 _slotId
    ) private pure returns (bool slotExists) {
