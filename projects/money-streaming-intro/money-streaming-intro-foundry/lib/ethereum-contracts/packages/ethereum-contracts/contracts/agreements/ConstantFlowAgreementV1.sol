 ...
 */
contract ConstantFlowAgreementV1 is AgreementBase, IConstantFlowAgreementV1 {
    /**
    ...
    function createFlow(
        ...
        newCtx = _createFlow(flowVars, ctx, currentContext);
    }
    ...
    function _createFlow(
        ...
        if (address(constantFlowAgreementHook) != address(0)) {
            ...
            try
                constantFlowAgreementHook.onCreate{ gas: CFA_HOOK_GAS_LIMIT }(
                    flowVars.token,
    ...
    function _updateFlow(
        ...
        if (address(constantFlowAgreementHook) != address(0)) {
            ...
            // solhint-disable-next-line no-empty-blocks
            try
                constantFlowAgreementHook.onUpdate{ gas: CFA_HOOK_GAS_LIMIT }(
                    flowVars.token,
                    IConstantFlowAgreementHook.CFAHookParams({
                        sender: flowParams.sender,
                        receiver: flowParams.receiver,
    ...
    function _deleteFlow(
        ...
        if (
            currentContext.msgSender != flowVars.sender &&
        ...
        /* liquidations or flowOperator deleting a flow */
        else {
        ...
        if (address(constantFlowAgreementHook) != address(0)) {
            uint256 gasLeftBefore = gasleft();
            try
                constantFlowAgreementHook.onDelete{ gas: CFA_HOOK_GAS_LIMIT }(
                    flowVars.token,
                    IConstantFlowAgreementHook.CFAHookParams({
                        sender: flowParams.sender,
                        receiver: flowParams.receiver,
    ...
    function _getAccountFlowState(
        ISuperfluidToken token,
        address account
    ) private view returns (bool exist, FlowData memory) {
    ...
    function _changeFlowToApp(
        ...
        if (appToCallback != address(0)) {
            ...
            ISuperfluidGovernance gov = ISuperfluidGovernance(
                ISuperfluid(msg.sender).getGovernance()
            );
        ...
        // and its range will be [0, currentContext.appCreditGranted]
        {
            ...
            if (ISuperfluid(msg.sender).isApp(ISuperApp(flowParams.receiver))) {
                ...
                (availableBalance, , ) = token.realtimeBalanceOf(
                    flowParams.receiver,
                    currentContext.timestamp
                );
                if (availableBalance < 0) {
    ...
    function _clipDepositNumberRoundingUp(
        uint256 deposit
    ) internal pure returns (uint256) {
    ...
    function _decodeFlowData(
        uint256 wordA
    ) internal pure returns (bool exist, FlowData memory flowData) {
        exist = wordA > 0;
        if (exist) {
