...
contract ConstantFlowAgreementV1Mock is ConstantFlowAgreementV1 {
    ...
    ) external pure returns (uint256 wordA) {
        return uint256(_encodeFlowData(flowData)[0]);
    }

    function decodeFlowData(
        uint256 wordA
    ) external pure returns (bool exist, FlowData memory flowData) {
        return _decodeFlowData(wordA);
    }

    function encodeFlowOperatorData(
        FlowOperatorData memory flowOperatorData
    ) external pure returns (uint256 wordA) {
        return uint256(_encodeFlowOperatorData(flowOperatorData)[0]);
    }

    function decodeFlowOperatorData(
        uint256 wordA
    )
        external
        pure
    ...
    function clipDepositNumberRoundingUp(
        uint256 deposit
    ...
    function getMaximumFlowRateFromDepositPure(
        ...
        uint256 deposit
    ) external pure returns (int96 flowRate) {
...
contract ConstantFlowAgreementV1Properties is Test {
    ...
    function testMaximumFlowRateAllowedForDeposit(
        ...
        depositAllowed = uint96(
            bound(
                uint256(depositAllowed),
                cfa.DEFAULT_MINIMUM_DEPOSIT(),
                cfa.MAXIMUM_DEPOSIT()
            )
        );
        vm.assume(liquidationPeriod > 0);

        int96 flowRate = cfa.getMaximumFlowRateFromDepositPure(
            liquidationPeriod,
            depositAllowed
        );
        assert(flowRate > 0);
        ...
        vm.assume(
            uint256(liquidationPeriod) * uint256(uint96(flowRate)) <=
                cfa.MAXIMUM_FLOW_RATE()
        );
        uint256 deposit = cfa.getDepositRequiredForFlowRatePure(
            0,
            liquidationPeriod,
            flowRate
        );
    ...
    function testMinimumDepositClippingSumInvariant(
        ...
        clippedDepositA = bound(
            clippedDepositA,
            0,
            type(uint256).max - clippedDepositB
        );
        clippedDepositB = bound(
            clippedDepositB,
            0,
            type(uint256).max - clippedDepositA
        );
        uint256 summedDeposit = clippedDepositA + clippedDepositB;
        uint256 clippedSummedDeposit = cfa.clipDepositNumberRoundingUp(
            summedDeposit
        );
    ...
    function testFlowDataEncoding(
        uint32 timestamp,
        int96 flowRate,
        uint64 depositClipped,
        uint64 owedDepositClipped
    ) public view {
        ConstantFlowAgreementV1.FlowData memory a = ConstantFlowAgreementV1
            .FlowData({
    ...
    function testFlowOperatorDataEncoding(
        uint8 permissions,
        int96 flowRateAllowance
    ) public {
        vm.assume(flowRateAllowance >= 0);
        ConstantFlowAgreementV1.FlowOperatorData
            memory a = ConstantFlowAgreementV1.FlowOperatorData({
