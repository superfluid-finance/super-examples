...
async function main() {
    ...
    try {
      ...
      let sf = await Framework.create({
       ...
       const vestingAutomator = await new VestingAutomation__factory(deployer).deploy(daix.address, gelatoAutomateMumbai, deployer.address, superfluidVestingSchedulerMumbai, { nonce: nonce  ,gasLimit: 10000000});

       let initialEth = hre.ethers.utils.parseEther('0.05');

       await deployer.sendTransaction({ to: vestingAutomator.address, value: initialEth, gasLimit: 10000000, nonce: nonce + 1});

        console.log('Vesting Automator deployed successfully at: ', vestingAutomator.address)
