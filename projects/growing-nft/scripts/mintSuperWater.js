...
async function main() {
  ...
  console.log(`Minting ${ethers.utils.formatUnits(mintAmount)} WATERx to ${signer.address}`)

  await water.connect(signer).mint( signer.address, mintAmount );
  console.log("minted water");

  let approveTx = await water.connect(signer).approve( waterx.address, mintAmount);
  ...
  console.log(`Success! ${signer.address} WATERx Balance: ${await waterx.balanceOf(signer.address)}`);
