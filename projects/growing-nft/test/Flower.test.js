...
describe("Flower Contract", function () {
    it("Creating a stream, token is minted", async function () {
        ...
        expect(await flower.balanceOf(bob.address)).to.eq(1)

        // Verify that it's recorded in `flowerOwned` mapping
        expect(await flower.flowerOwned(bob.address)).to.eq(1)

        // Verify that metadata is [0]
        expect(await flower.tokenURI("1")).to.eq(
            "ipfs://QmYUXy3JjoCjx1Fji71v9pPAWs3kAdrhBtUvVJw6m89g4A/plant1.json"
        )
    })
    ...
    it("Updating a stream", async function () {
        ...
        expect(amountStreamedToFlower).is.closeTo(
            timeIncrease * 2,
    ...
    it("Transfering the NFT", async function () {
        ...
        expect(await flower.flowerOwned(bob.address)).to.eq(0)
        expect(await flower.flowerOwned(alice.address)).to.eq(1)
        ...
        expect(flowerProf.latestFlowMod).to.eq(trackedTime)
        expect(flowerProf.flowRate).to.eq(0)
        expect(flowerProf.streamedSoFarAtLatestMod).is.closeTo(
            12100,
            EXPECATION_DIFF_LIMIT
        )
    ...
    it("Account that previously had an NFT can mint another", async function () {
        ...
        // Verify that metadata is [0]
        expect(await flower.tokenURI("2")).to.eq(
            "ipfs://QmYUXy3JjoCjx1Fji71v9pPAWs3kAdrhBtUvVJw6m89g4A/plant1.json"
        )
    })
    ...
    it("Account can't have multiple NFTs", async function () {
        ...
        ).to.be.revertedWithCustomError(flower, "InvalidTransfer")
    })
