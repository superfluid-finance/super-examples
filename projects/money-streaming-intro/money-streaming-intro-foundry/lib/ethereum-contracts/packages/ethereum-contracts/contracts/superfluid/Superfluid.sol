 ...
 * @author Superfluid
 */
contract Superfluid is UUPSProxiable, ISuperfluid, BaseRelayRecipient {
     ...
     */
    // solhint-disable-next-line var-name-mixedcase
    uint internal constant MAX_APP_CALLBACK_LEVEL = 1;

    // solhint-disable-next-line var-name-mixedcase
    uint64 public constant CALLBACK_GAS_LIMIT = 3000000;
    ...
    function updateCode(address newAddress) external override onlyGovernance {
        ...
        if (Superfluid(newAddress).NON_UPGRADABLE_DEPLOYMENT())
    ...
    ) external view override returns (bool yes) {
        uint idx = _agreementClassIndices[agreementType];
        return idx != 0;
    }
    ...
    function isAgreementClassListed(
        ISuperAgreement agreementClass
    ) public view override returns (bool yes) {
    ...
    function getAgreementClass(
        bytes32 agreementType
    ) external view override returns (ISuperAgreement agreementClass) {
        uint idx = _agreementClassIndices[agreementType];
        if (idx == 0) {
            revert HOST_AGREEMENT_IS_NOT_REGISTERED();
        }
    ...
    function _registerApp(
        ...
        if (_appManifests[ISuperApp(app)].configWord != 0)
            revert HOST_SUPER_APP_ALREADY_REGISTERED();
    ...
    ) external override onlyAgreement returns (bytes memory newCtx) {
        Context memory context = decodeCtx(ctx);
    ...
    function callAppAction(
        ISuperApp app,
        bytes memory callData // NOTE: modifiers are called in _callAppAction
    ) external override returns (bytes memory returnedData) {
    ...
    {
        Context memory context = decodeCtx(ctx);
        if (context.appAddress != msg.sender)
            revert HOST_CALL_AGREEMENT_WITH_CTX_FROM_WRONG_ADDRESS();
        ...
        returns (bytes memory newCtx)
    {
        Context memory context = decodeCtx(ctx);
        if (context.appAddress != msg.sender)
            revert HOST_CALL_APP_ACTION_WITH_CTX_FROM_WRONG_ADDRESS();
    ...
    function decodeCtx(
        bytes memory ctx
    ) public pure override returns (Context memory context) {
    ...
    function _updateContext(
    ...
    ) private returns (bytes memory ctx) {
        ...
        uint256 creditIO = context.appCreditGranted.toUint128() |
    ...
    function _decodeCtx(
        bytes memory ctx
    ) private pure returns (Context memory context) {
        bytes memory ctx1;
        bytes memory ctx2;
    ...
    function _callExternalWithReplacedCtx(
        ...
        bytes memory callData,
        bytes memory ctx
    ) private returns (bool success, bytes memory returnedData) {
    ...
    function _replacePlaceholderCtx(
        bytes memory data,
        bytes memory ctx
    ) internal pure returns (bytes memory dataWithCtx) {
    ...
    modifier cleanCtx() {
        if (_ctxStamp != 0)
            revert APP_RULE(SuperAppDefinitions.APP_RULE_CTX_IS_NOT_CLEAN);
        _;
    }
