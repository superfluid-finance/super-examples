...
contract FoundrySuperfluidTester is Test {
    ...
    address[] internal TEST_ACCOUNTS = [
        admin,
        alice,
        bob,
        carol,
    ...
    function checkAllInvariants() public view returns (bool) {
        return checkLiquiditySumInvariance() && checkNetFlowRateSumInvariant();
    }

    function checkLiquiditySumInvariance() public view returns (bool) {
        int256 liquiditySum;
        for (uint i = 0; i < TEST_ACCOUNTS.length; ++i) {
            (int256 avb, uint256 d, uint256 od, ) = superToken
                .realtimeBalanceOfNow(address(TEST_ACCOUNTS[i]));
